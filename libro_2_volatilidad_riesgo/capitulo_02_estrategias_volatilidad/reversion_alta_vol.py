...
def estrategia_reversion_alta_volatilidad(df, periodo_ma=50, umbral_volatilidad=0.75, 
    ...
    for i in range(periodo_ma, len(df)):
        ...
        # Señal de venta (sobrecompra) - precio cruza banda superior hacia abajo
        if precio > banda_sup and df['close'].iloc[i-1] > banda_sup:
            if precio <= banda_sup:
        ...
        # Señal de compra (sobreventa) - precio cruza banda inferior hacia arriba
        elif precio < banda_inf and df['close'].iloc[i-1] < banda_inf:
            if precio >= banda_inf:
